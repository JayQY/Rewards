...
(function () {
    ...
    if (window.location.href.indexOf("rewards.bing.com") != -1) {
        ...
        function runToday0() {
            ...
            for (var i = 0; i < list.length; i++) {
                document.querySelectorAll('a.ds-card-sec')[i].click();
            }
        }
    ...
    if (window.location.href.indexOf("www.bing.com") != -1) {
        if ($('#ListOfQuestionAndAnswerPanes').length > 0) {
            var thisObj = $('#ListOfQuestionAndAnswerPanes').find('div.wk_OptionClickClass');
            if (thisObj.length > 0) {
                setInterval(() => {
                    var n = randomNum(0, thisObj.length - 1);
                    thisObj.eq(n).click();
                }, clickTimeout);
                //thisObj.eq(0).click();
            }
            //完成
            //var thisButton = $('#ListOfQuestionAndAnswerPanes').find('input[value="Get your score"]');
            //var thisA = thisButton.parentsUntil('a');
            //var thisDiv = thisButton.parents('div.wk_buttons');
        ...
        if ($('.btOptionCard').length > 0) {
            setInterval(() => {
                var k = randomNum(0, 1);
                $('.btOptionCard').eq(k).click();
            }, clickTimeout);
        }
        ...
        if ($('.b_cards:not(.btsel)').length > 0) {
            setInterval(() => {
                $('.b_cards:not(.btsel)').click();
            }, clickTimeout);
        }

        if ($(".rqOption").length > 0) {
            setInterval(() => {
                $(".rqOption").click();
            }, clickTimeout);
        }
